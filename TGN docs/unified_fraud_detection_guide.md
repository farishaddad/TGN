# AI-Powered Fraud Detection Using Temporal Graphs and Machine Learning
...
4. [TGN Model Architecture — Heterogeneous](#4-tgn-model-architecture)
5. [Multi-Source Data Fusion Pipeline](#5-multi-source-data-fusion-pipeline)
6. [Multi-Bank Integration Patterns](#6-multi-bank-integration-patterns)
7. [Entity Resolution Across Institutions](#7-entity-resolution-across-institutions)
8. [Incremental Learning & Concept Drift](#8-incremental-learning--concept-drift)
9. [Risk Quantification Framework](#9-risk-quantification-framework)
10. [Production Deployment & Real-Time Scoring](#10-production-deployment--real-time-scoring)
11. [Rules Engine Integration](#11-rules-engine-integration)
12. [Cold-Start Problem Solutions](#12-cold-start-problem-solutions)
13. [Transfer Learning Extensibility](#13-transfer-learning-extensibility)
14. [AWS Demo Architecture & Deployment](#14-aws-demo-architecture--deployment)
15. [Implementation Roadmap](#15-implementation-roadmap)
16. [Business Case & ROI](#16-business-case--roi)
- [Appendix A: Inference Latency Analysis](#appendix-a-inference-latency-analysis)
- [Appendix B: Mean vs Last Aggregator Rationale](#appendix-b-mean-vs-last-aggregator-rationale)
...
Financial fraud costs the global economy over $5 trillion annually. Traditional rule-based systems catch known patterns but fail against evolving threats. Fraudsters exploit both network relationships AND behavioural patterns that shift dynamically — requiring a system that understands both dimensions simultaneously.

### Our Solution: Three Core Capabilities

**1. The Relationship Map (Temporal Graphs)**

Every transaction, login, and device interaction is captured as a connection in a continuously-evolving graph. Unlike static databases, this reveals:
- Who transacts with whom, when, and how patterns shift over time
- Network-level fraud patterns (mule networks, fraud rings) invisible to per-account analysis
- Temporal dynamics — sudden behaviour changes that precede fraud

**2. The Learning System (Heterogeneous Temporal Graph Networks)**

A Temporal Graph Network with per-type memory banks learns entity-specific representations that update with each interaction:
- **Account memory** (128d) tracks spending behaviour evolution
- **Device memory** (64d) tracks usage patterns and geolocation drift
- **Merchant memory** (64d) tracks volume patterns and complaint signals
- **Threat indicator memory** (32d) stores external intelligence embeddings

The model uses relational attention — attending differently to transfer neighbours vs device-sharing neighbours vs alert neighbours — capturing type-specific patterns structurally.

**3. The Connected Intelligence (Multi-Bank Integration)**

A federated hub-and-spoke architecture connects multiple banking backends, third-party data, and dark web intelligence feeds:
- Privacy-preserving entity resolution across institutions
- Encrypted matching without sharing raw customer data
- Cross-bank fraud ring detection impossible for any single institution

### How It Works: A Complete Example

**Scenario: Your Card Gets Stolen**

11:30 PM Tuesday: Thief steals card details from a restaurant data breach.
2:00 AM Wednesday: Thief tries to buy £2,000 laptop online.

What happens behind the scenes (takes ~15ms):

1. **Fast Path Check** (2ms): Card not on sanctions list ✓, under daily limit ✓, quick ML says 52% risk — uncertain. Decision: needs deeper look.

2. **TGN Deep Analysis** (13ms):
   - Time Analysis: "You never shop at 2 AM" ⚠️
   - Location: "New city, 200 miles from home" ⚠️
   - Device: "Never seen this device before" ⚠️ (login edge creates new device node)
   - Network: "This merchant had 3 fraud reports today" ⚠️ (graph neighbourhood signal)
   - Relational attention: Cross-type convergence of device anomaly + time anomaly + merchant risk
   - **Result: 94% fraud probability → BLOCKED**

### Financial Impact

| Metric | Before | After (Year 1) | After (Year 2) |
|--------|--------|-----------------|-----------------|
| Fraud loss rate | Baseline | -35% | -50%+ |
| False positive rate | Baseline | -50% | -70%+ |
| Decision latency (p50) | 200ms+ | 15ms | 12ms |
| Cross-bank rings detected | 0 | 5+ | 20+ |
| Analyst productivity | Baseline | +40% | +60% |

### Strategic Advantages

- **Network effects**: Each additional institution strengthens detection for all participants
- **Adaptive**: Continuous learning from new fraud patterns without full retraining
- **Explainable**: Every decision traces back to specific graph signals and temporal patterns
- **Privacy-first**: Federated architecture ensures no institution shares raw customer data
...
## 4. TGN Model Architecture — Heterogeneous
...
## 6. Multi-Bank Integration Patterns

### Federated Graph Hub-and-Spoke Architecture

The system uses a federated architecture where each banking institution retains sovereignty over their data while contributing to a shared intelligence layer:

```
┌──────────────────────────────────────────────────────────┐
│          Unified Fraud Detection Layer                     │
│  (Heterogeneous TGN + ML Models + Inference)              │
└────────────────────────────┬─────────────────────────────┘
                             │
┌────────────────────────────┴─────────────────────────────┐
│       Graph Fusion & Entity Resolution Engine             │
│   - Probabilistic matching across sources                 │
│   - Temporal alignment & synchronisation                  │
│   - Meta-graph construction                               │
└──────┬──────────┬──────────┬───────────┬─────────────────┘
       │          │          │           │
  ┌────▼────┐ ┌──▼────┐ ┌──▼────┐ ┌───▼──────┐
  │ Bank A  │ │Bank B │ │ 3rd   │ │Dark Web  │
  │ Adapter │ │Adapter│ │ Party │ │ Feed     │
  └─────────┘ └───────┘ └───────┘ └──────────┘
```

### Adapter Layer Pattern

Each data source implements a lightweight adapter that translates to the canonical `UnifiedGraphEvent` schema:
...
class DataSourceAdapter(ABC):
    """
    Abstract adapter for integrating any data source into the graph.
    Each institution implements this interface.
    """
    
    @abstractmethod
    def poll_events(self, since: datetime) -> Iterator['UnifiedGraphEvent']:
        """Stream new events since timestamp."""
        pass
    
    @abstractmethod
    def get_entity_context(self, entity_id: str) -> 'EntityContext':
        """Retrieve historical context for entity resolution."""
        pass
    
    def get_capabilities(self) -> 'SourceCapabilities':
        """Declare what this source can provide."""
        return SourceCapabilities(
            entity_types=["account", "device"],
            relationship_types=["transaction", "login"],
            temporal_resolution="millisecond",
            lookback_window_days=90,
        )


class DarkWebAdapter(DataSourceAdapter):
    """
    Ingests threat intelligence from dark web monitoring:
    - Compromised credential dumps
    - Stolen card databases
    - Mule recruitment forums
    - Fraud-as-a-service listings
    """
    
    def poll_events(self, since: datetime):
        for intel_item in self._fetch_threat_intel(since):
            entities = self._extract_entities(intel_item)
            yield UnifiedGraphEvent(
                event_type=EventType.THREAT_INDICATOR,
                src_node_type='threat_indicator',
                dst_node_type='account',
                edge_type='threat_link',
                edge_features=self._encode_threat(intel_item),
                confidence=intel_item.confidence,
                source_system='darkweb',
            )
```

### Dynamic Schema Extension

When a new data source connects, the system extends its schema without retraining:

```python
class DynamicSchemaManager:
    def add_new_source(self, source_adapter: DataSourceAdapter):
        """
        Integrate new data source without retraining:
        1. Extend graph schema (new node/edge types)
        2. Initialize embeddings for new types
        3. Fine-tune model on historical data from new source
        """
        capabilities = source_adapter.get_capabilities()
        
        # Extend node registry if new types
        for entity_type in capabilities.entity_types:
            if entity_type not in self.node_registry.type_configs:
                self._add_node_type(entity_type)
        
        # Warm-start training
        historical_batch = source_adapter.get_historical_data(lookback_days=30)
        self._warm_start_training(historical_batch)
    
    def _warm_start_training(self, historical_batch):
        """
        Fine-tune on new source:
        - Phase 1: Freeze existing parameters, train only new components
        - Phase 2: Gradually unfreeze all parameters with low learning rate
        """
        # Phase 1: Train new components only
        for param in self.model.parameters():
            param.requires_grad = False
        
        for node_type in self.schema.new_types:
            for param in self.model.memory.grus[node_type].parameters():
                param.requires_grad = True
        
        self._train_epochs(historical_batch, epochs=5, lr=1e-3)
        
        # Phase 2: Unfreeze all with low lr
        for param in self.model.parameters():
            param.requires_grad = True
        
        self._train_epochs(historical_batch, epochs=10, lr=1e-5)
```

### Integration with Heterogeneous TGN

The adapter architecture maps directly to the TGN's type system:

| Source | Produces Node Types | Produces Edge Types | Edge Type Index |
|--------|-------------------|-------------------|-----------------|
| Bank A/B transfers | account | transfer | 0 |
| Authentication system | account, device | login | 1 |
| Card processor | account, merchant | purchase | 2 |
| Rules engine alerts | account | alert | 3 |
| Dark web feed | threat_indicator, account | threat_link | 4 |
| Device fingerprinting | device | device_shared | 5 |

Each adapter's output is a `UnifiedGraphEvent` with explicit `src_node_type`, `dst_node_type`, and `edge_type` — enabling the TGN's per-type memory banks and typed message functions to process them appropriately.

---

## 7. Entity Resolution Across Institutions

### The Challenge

Entity resolution across banking systems is complex because:
- No universal identifier exists across institutions
- Deliberate obfuscation by fraudsters
- Privacy constraints prevent sharing raw customer data
- Temporal drift (people change addresses, phone numbers)
- Scale (billions of transactions, millions of entities)

### Four-Tier Resolution Strategy

#### Tier 1: Cryptographic Matching (Deterministic)

```python
class CryptographicMatcher:
    """
    Hash strong identifiers for privacy-preserving exact matching.
    Uses Bloom filters for fast negative lookups.
    """
    
    def __init__(self, hash_function='sha256', salt_per_bank=True):
        self.hash_fn = hash_function
        self.salt_per_bank = salt_per_bank
        self.bloom_filter = BloomFilter(capacity=1e9, error_rate=0.001)
    
    def create_identifier_token(self, identifier: str, source: str) -> str:
        """
        Create privacy-preserving tokens:
        - Hash PII with source-specific salt
        - Store only hashes, never plaintext
        """
        salt = self._get_source_salt(source) if self.salt_per_bank else self.shared_salt
        return hmac.new(salt.encode(), identifier.encode(), self.hash_fn).hexdigest()
    
    def find_matches(self, bank_a_set: set, bank_b_set: set) -> set:
        """
        Private Set Intersection using double-encryption:
        Neither bank reveals their full set to the other.
        """
        key_a = self._generate_private_key()
        encrypted_a = {self._encrypt(hash(item), key_a) for item in bank_a_set}
        
        key_b = self._generate_private_key()
        double_encrypted_a = {self._encrypt(enc, key_b) for enc in encrypted_a}
        
        encrypted_b = {self._encrypt(hash(item), key_b) for item in bank_b_set}
        double_encrypted_b = {self._encrypt(enc, key_a) for enc in encrypted_b}
        
        # Intersection without revealing which items matched
        return double_encrypted_a & double_encrypted_b
```

#### Tier 2: Probabilistic Matching (Fellegi-Sunter)

```python
class ProbabilisticMatcher:
    """
    Fellegi-Sunter model for fuzzy matching:
    ...
    def compute_match_score(self, entity_a, entity_b) -> float:
        """
        Compute log-likelihood ratio:
        score = log(P(observations | match) / P(observations | non-match))
        """
        log_likelihood_ratio = 0
        features = self._extract_features(entity_a, entity_b)
        
        for feature, agrees in features.items():
            m = self.m_probabilities[feature]  # P(agree | match)
            u = self.u_probabilities[feature]  # P(agree | non-match)
            
            if agrees:
                log_likelihood_ratio += np.log(m / u)
            else:
                log_likelihood_ratio += np.log((1 - m) / (1 - u))
        
        return log_likelihood_ratio
```

#### Tier 3: ML-Based Matching

```python
class NeuralEntityMatcher(nn.Module):
    """
    Deep learning for complex entity matching using:
    - Device usage patterns
    - Behavioural sequence matching
    - Network context similarity
    """
    
    def __init__(self, embedding_dim=128):
        super().__init__()
        self.device_encoder = DeviceFingerprintEncoder(embedding_dim)
        self.behavior_encoder = BehaviorSequenceEncoder(embedding_dim)
        self.network_encoder = NetworkContextEncoder(embedding_dim)
        
        # Cross-attention to find correspondences
        self.cross_attention = nn.MultiheadAttention(
            embed_dim=embedding_dim, num_heads=8
        )
    ...
    def forward(self, entity_a, entity_b):
        device_a = self.device_encoder(entity_a.device_fingerprints)
        behavior_a = self.behavior_encoder(entity_a.transaction_history)
        network_a = self.network_encoder(entity_a.graph_neighbourhood)
        
        device_b = self.device_encoder(entity_b.device_fingerprints)
        behavior_b = self.behavior_encoder(entity_b.transaction_history)
        network_b = self.network_encoder(entity_b.graph_neighbourhood)
        
        # Cross-attention finds correspondences
        fused, _ = self.cross_attention(
            torch.stack([device_a, behavior_a, network_a]),
            torch.stack([device_b, behavior_b, network_b]),
            torch.stack([device_b, behavior_b, network_b]),
        )
...
#### Tier 4: Temporal Validation

```python
class TemporalValidator:
    """
    Final validation using temporal constraints:
    - Geographic impossibilities (can't be in two cities at once)
    - Device usage patterns must be consistent
    - Account creation sequences must make sense
    """
    
    def validate_match(self, entity_a, entity_b, proposed_match_score):
        # Check geographic impossibility
        if self._is_geographically_impossible(entity_a, entity_b):
            return False
        
        # Validate device consistency
        if not self._device_patterns_consistent(entity_a, entity_b):
            return proposed_match_score * 0.5  # Downgrade confidence
        
        # Verify temporal plausibility
        if not self._temporal_sequence_valid(entity_a, entity_b):
            return False
        
        return proposed_match_score
```

### Privacy-Preserving Cross-Bank Resolution

The entity resolution system uses a privacy-preserving protocol where:
1. Banks share only **hashed identifiers** and **encrypted embeddings**
2. The fusion engine matches on encrypted representations
3. When enough signals converge, entities are linked **without revealing PII**
4. Each bank retains full sovereignty over their customer data

---

## 8. Incremental Learning & Concept Drift

### Temporal Graph Update Strategy

The system updates continuously without requiring full retraining:

```python
class IncrementalTemporalGraph:
    """
    Manages the evolving temporal graph with:
    - Real-time edge insertion
    - Memory management (LRU eviction of inactive nodes)
    - Statistical aggregations for historical context
    """
    ...
    def __init__(self, memory_size=100_000):
        ...
        self.node_memory = LRUCache(maxsize=memory_size)
    ...
    def add_transaction(self, transaction):
        ...
        - Add nodes if new
        - Add temporal edge
        - Update node embeddings via GRU
        - Manage memory capacity
        """
        src_id = transaction.from_account
        dst_id = transaction.to_account
        timestamp = transaction.timestamp
        
        if src_id not in self.nodes:
            self._add_new_node(src_id, transaction.src_metadata)
        if dst_id not in self.nodes:
            self._add_new_node(dst_id, transaction.dst_metadata)
        
        # Add temporal edge
        edge_id = (src_id, dst_id, timestamp)
        ...
        # Update time index for temporal queries
        self.time_index[timestamp] = edge_id
        
        # Update aggregated statistics (rolling means, velocities)
        self._update_statistics(src_id, transaction)
        self._update_statistics(dst_id, transaction)
        
        # Memory management
        if len(self.nodes) > self.memory_size:
            self._evict_inactive_nodes()
    
    def _update_statistics(self, node_id, transaction):
        """Update rolling aggregated statistics for the node."""
        if node_id not in self.node_statistics:
            self.node_statistics[node_id] = NodeStats()
        
        stats = self.node_statistics[node_id]
        stats.update(
            amount=transaction.amount,
            timestamp=transaction.timestamp,
...
The TGN architecture is inherently incremental — the GRU memory updater processes each batch sequentially, naturally incorporating new patterns:

```python
class IncrementalModelManager:
    """
    Manages continuous model improvement:
    - Online memory updates (every batch)
    - Periodic parameter fine-tuning (daily/weekly)
    - Full retraining triggered only by concept drift
    """
    
    def __init__(self, model: 'HeteroTGNFraudDetector', drift_detector):
        self.model = model
        self.drift_detector = drift_detector
        self.update_buffer = []
        self.fine_tune_interval = 86400  # Daily
        self.last_fine_tune = time.time()
    
    def process_batch(self, batch):
        """
        Online update: memory states evolve with each batch.
        No parameter updates — just memory evolution via GRU.
        """
        with torch.no_grad():
            self.model.eval()
            output = self.model(
                batch.src, batch.dst, batch.t,
                batch.msg, batch.edge_type,
            )
        
        # Store predictions for drift detection
        self.update_buffer.append({
            'predictions': output['node_scores'].cpu(),
            'timestamps': batch.t.cpu(),
            'multiplicities': self._compute_multiplicities(batch),
        })
        
        return output
    
    def check_and_adapt(self):
        """Periodic check for drift + fine-tuning."""
        if time.time() - self.last_fine_tune < self.fine_tune_interval:
            return
        
        # Check for concept drift
        drift_detected, signals = self.drift_detector.detect(
            predictions=self._get_recent_predictions(),
            labels=self._get_recent_labels(),
            batch_multiplicities=self._get_multiplicities(),
        )
        
        if drift_detected:
            self._trigger_adaptation(signals)
        else:
            self._fine_tune_incremental()
...
Fraud patterns evolve as adversaries adapt. The system monitors for drift using multiple signals:

```python
class ConceptDriftDetector:
    """
    Multi-method drift detection combining:
    - Performance degradation (F1/precision drops)
    - Prediction distribution shift (Kolmogorov-Smirnov test)
    - Feature distribution shift
    - Batch multiplicity changes (aggregation-specific signal)
    
    Triggers adaptation when ≥2 signals fire simultaneously.
    """
    
    def __init__(self):
        # ADWIN: Adaptive Windowing for performance monitoring
        self.adwin = ADWIN()
        self.ks_test_window = 1000
        self.reference_distribution = None
        self.multiplicity_baseline = None
    
    def detect(self, predictions, labels, batch_multiplicities=None):
        """
        Returns (drift_detected: bool, signals: list).
        Drift is declared when ≥2 independent signals fire.
        """
        drift_signals = []
        ...
        if self._detect_prediction_drift(predictions):
            drift_signals.append('prediction_distribution_shift')
        
        # Signal 3: Feature distribution shift
        if self._detect_feature_drift(predictions):
            drift_signals.append('feature_distribution_shift')
        
        # Signal 4: Batch multiplicity change (mean-aggregator-specific)
        # If average messages-per-node-per-batch changes significantly,
        # the aggregated message semantics shift
        if batch_multiplicities is not None:
            if self._detect_multiplicity_drift(batch_multiplicities):
                drift_signals.append('multiplicity_shift')
        
        return len(drift_signals) >= 2, drift_signals
    
    def _detect_performance_drift(self, predictions, labels):
        """ADWIN-based detection of accuracy degradation."""
        if labels is None:
            return False
        
        errors = (predictions.round() != labels).float()
        for error in errors:
            self.adwin.add_element(error.item())
        
        return self.adwin.detected_change()
    
    def _detect_prediction_drift(self, predictions):
        """Kolmogorov-Smirnov test for distribution shift."""
        if self.reference_distribution is None:
            self.reference_distribution = predictions[-self.ks_test_window:]
            return False
        
        from scipy.stats import ks_2samp
        recent = predictions[-self.ks_test_window:]
        stat, p_value = ks_2samp(
            self.reference_distribution.numpy(),
            recent.numpy(),
        )
    ...
    def _detect_multiplicity_drift(self, multiplicities):
        """
        Detect when batch multiplicity patterns change.
        
        This is specific to mean aggregation: if nodes suddenly receive
        more/fewer messages per batch, the GRU input semantics shift
        (average-of-5 has different magnitude than average-of-50).
        """
        if self.multiplicity_baseline is None:
            self.multiplicity_baseline = multiplicities[-self.ks_test_window:]
            return False
        
        from scipy.stats import ks_2samp
        stat, p_value = ks_2samp(
            self.multiplicity_baseline,
            multiplicities[-self.ks_test_window:],
        )
        return p_value < 0.01
```
...
When a new institution joins or a new data feed is connected:

1. **Schema extension**: New node/edge types added to `HeteroNodeRegistry`
2. **Memory allocation**: New memory banks initialised with appropriate dimensions
3. **Warm-start training**: Freeze existing weights, train new components on 30-day history
4. **Gradual unfreezing**: Fine-tune full model with very low learning rate (1e-5)
5. **Validation**: Compare performance before/after on held-out test set

This process takes 2-3 days from integration to production — not the months required by full retraining approaches.

---

## 9. Risk Quantification Framework
...
## 10. Production Deployment & Real-Time Scoring

### Two-Tier Inference Architecture

Real-time fraud detection requires balancing speed against depth. The system uses an intelligent routing pattern:
...
│ - Cached          │                      │ - Full graph        │
│   embeddings      │                      │   context           │
│ - Rule engine     │                      │ - Multi-hop         │
│ - Lightweight ML  │                      │   aggregation       │
│ - Quantised TGN   │                      │ - Complex temporal  │
│                   │                      │   pattern analysis  │
└────────┬──────────┘                      └──────────┬──────────┘
         │                                             │
         └──────────────────┬──────────────────────────┘
...
For routine transactions where the pre-computed risk profile is low:

```python
class FastPathScorer:
    """
    Strict latency budget: p50 <15ms, p99 <50ms
    
    Budget breakdown:
    - 2ms: Fetch cached embeddings (3-tier: local → Redis → compute)
    - 3ms: Rule evaluation
    - 8ms: TGN inference (GPU, batched)
    - 2ms: Buffer
    """
    
    async def score_transaction(self, transaction, timeout_ms=50):
        start = time.time()
        
        try:
            # Tier 1: Local LRU cache (microseconds)
            # Tier 2: Redis (1-2ms)
            # Tier 3: Compute fresh (8-15ms)
            async with timeout(0.015):
                embeddings = await self._fetch_embeddings(transaction)
            
            # Rule-based quick wins
            rule_result = self.rules.evaluate_fast_rules(transaction, embeddings)
            ...
            # TGN inference (quantised model on GPU)
            tgn_score = await self._tgn_inference(transaction, embeddings)
            ...
            # Fallback: use most recent cached score
            return self._fallback_score(transaction)
```

### Deep Path (5% Suspicious Cases)

Triggered when the fast path produces uncertain scores (0.3–0.7 range) or when specific patterns warrant deeper analysis:

```python
class DeepPathAnalyzer:
    """
    Full graph context retrieval and multi-hop analysis.
    Latency budget: <5 seconds (async, doesn't block payment).
    """
    
    async def analyze(self, transaction, fast_path_score):
        # Full temporal neighbourhood retrieval (multi-hop)
        context = await self._get_full_graph_context(
            entity_id=transaction.source_account,
            hops=3,
            time_window_hours=168,  # 1 week
        )
        
        # Complex temporal pattern analysis
        patterns = self._detect_temporal_patterns(context)
        
        # Full TGN inference with extended neighbourhood
        full_embedding = self._compute_deep_embedding(
            transaction, context, num_neighbors=50
        )
        
        # Ensemble decision
        deep_score = self._ensemble_predict(
            fast_score=fast_path_score,
            deep_embedding=full_embedding,
            patterns=patterns,
        ...
        return DeepAnalysisResult(
            ...
            contributing_factors=self._explain(full_embedding),
            recommendation=self._determine_action(deep_score),
        )
```
...
class InferenceRouter:
    """
    Decides fast path vs deep path based on:
    - Quick risk estimation from cached embeddings
    - Transaction characteristics (amount, time, counterparty)
    - Account risk tier
    """
    
    def route(self, transaction) -> str:
        # Known low-risk patterns: fast path
        if transaction.amount < self.low_amount_threshold:
            return 'fast'
        if transaction.destination in self.trusted_merchants:
            return 'fast'
        
        # Known high-risk patterns: deep path
        if transaction.source_account.risk_tier == 'HIGH':
            return 'deep'
        if transaction.is_cross_border and transaction.amount > 5000:
            return 'deep'
        
        # Uncertain: start with fast, escalate if score is ambiguous
        return 'fast_with_escalation'
```
...
class EmbeddingCache:
    """
    Three-tier caching for ultra-low latency embedding retrieval:
    - Tier 1: Local LRU (microseconds, 10K entries)
    - Tier 2: Redis (1-2ms, 1M entries)
    - Tier 3: Compute fresh from TGN memory (8-15ms)
    """
    ...
    async def get_embedding(self, entity_id, entity_type):
        cache_key = f"{entity_type}:{entity_id}"
        
        # Tier 1: Local memory (microseconds)
        ...
        # Tier 3: Compute fresh (8-15ms)
        embedding = await self._compute_embedding(entity_id, entity_type)
...
|-----------|-----|-----|-----|
| Kafka consume + deserialise | 1ms | 2ms | 3ms |
| Feature encoding | 0.5ms | 0.8ms | 1ms |
| Node ID resolution | <0.1ms | <0.1ms | <0.1ms |
| **TGN Inference (GPU, batch=50)** | **5ms** | **10ms** | **15ms** |
| Risk quantification (XGBoost) | 0.5ms | 1ms | 2ms |
| Feature store write (Redis) | 1ms | 1.5ms | 2ms |
| Rules engine evaluation | 1ms | 2ms | 3ms |
| Kafka produce (result) | 1ms | 1.5ms | 2ms |
| **TOTAL** | **~12ms** | **~20ms** | **~30ms** |

### Model Quantisation for Fast Path

```python
class QuantisedTGN:
    """
    INT8/FP16 quantised version of the scoring heads for fast path.
    The relational attention is kept in FP32 (accuracy-sensitive).
    """
    
    @classmethod
    def from_full_model(cls, model: HeteroTGNFraudDetector):
        quantised = cls()
        quantised.scorer = torch.quantization.quantize_dynamic(
            model.scorer,
            {nn.Linear},
            dtype=torch.qint8,
        )
        # Keep attention in full precision
        quantised.gnn = model.gnn
        return quantised
```
...
## 12. Cold-Start Problem Solutions

### The Challenge

New customers have zero transaction history. The TGN's memory banks are empty — no learned embeddings exist. How do we protect the institution from fraud without rejecting legitimate new customers?

### Five-Strategy Solution

#### Strategy 1: Transfer Learning from Similar Entities

```python
class TransferLearningBootstrap:
    """
    Find the K most similar known entities and initialise the new
    entity's memory as a weighted average of their states.
    """
    
    def bootstrap_new_entity(self, new_entity_metadata: dict) -> torch.Tensor:
        # Encode metadata into a feature vector
        meta_features = self._encode_metadata(new_entity_metadata)
        
        # Find K nearest entities in the existing population
        # based on demographics, account type, geography
        similar_entities = self.entity_index.search(
            meta_features, k=10
        )
        
        # Weighted average of their current memory states
        weights = F.softmax(similar_entities.scores, dim=0)
        initial_memory = torch.zeros(self.memory_dim)
        
        for entity_id, weight in zip(similar_entities.ids, weights):
            entity_memory = self.model.memory.get_memory(entity_id)
            initial_memory += weight * entity_memory
        
        return initial_memory
```

#### Strategy 2: Progressive Trust Building

```python
class ProgressiveTrustManager:
    """
    Gradually increase transaction limits as the entity builds history.
    Risk thresholds start conservative and relax with demonstrated behaviour.
    """
    
    TRUST_TIERS = {
        'new': {'daily_limit': 100, 'extra_verification': True, 'deep_path_always': True},
        'week_1': {'daily_limit': 500, 'extra_verification': True, 'deep_path_always': False},
        'month_1': {'daily_limit': 2000, 'extra_verification': False, 'deep_path_always': False},
        'month_6': {'daily_limit': 10000, 'extra_verification': False, 'deep_path_always': False},
        'established': {'daily_limit': 50000, 'extra_verification': False, 'deep_path_always': False},
    }
    
    def get_tier(self, entity_id) -> dict:
        history = self.get_entity_history(entity_id)
        if history.age_days < 7:
            return self.TRUST_TIERS['new']
        elif history.age_days < 30:
            return self.TRUST_TIERS['week_1']
        elif history.age_days < 180:
            return self.TRUST_TIERS['month_1']
        elif history.clean_record:
            return self.TRUST_TIERS['established']
        return self.TRUST_TIERS['month_6']
```

#### Strategy 3: External Signal Integration

Leverage signals available even for brand-new customers:
- **Email age**: 5-year-old email = good sign; created yesterday = red flag
- **Phone number**: Real mobile line = good; disposable VoIP = red flag
- **Device reputation**: Well-known model = normal; rooted/jailbroken = suspicious
- **IP address**: Home broadband = normal; TOR/VPN = needs watching

```python
class ExternalSignalScorer:
    def score_new_entity(self, metadata: dict) -> float:
        signals = []
        
        # Email reputation
        email_age = self.email_api.get_age(metadata.get('email'))
        signals.append(min(email_age / 365, 1.0))  # Normalise to [0,1]
        
        # Phone validation
        phone_valid = self.phone_api.validate(metadata.get('phone'))
        signals.append(1.0 if phone_valid else 0.0)
        
        # Device trust score
        device_score = self.device_api.get_trust(metadata.get('device_fingerprint'))
        signals.append(device_score)
        
        # IP reputation
        ip_score = self.ip_api.get_reputation(metadata.get('ip_address'))
        signals.append(ip_score)
        
        return np.mean(signals)
```

#### Strategy 4: Behavioural Fingerprinting

Even without transaction history, behavioural biometrics provide identity signals:
- Typing speed, rhythm, and correction patterns
- Mouse/touch movement patterns
- Form-filling behaviour (time per field, field ordering)
- These persist across accounts — a fraudster opening their 5th fake account still types the same way

#### Strategy 5: Network-Based Bootstrap

The first interaction reveals graph context:
- First transaction to established Amazon merchant → low risk
- First transaction to another brand-new account → high risk
- Device shared with 3 existing accounts → inherit their risk profile

```python
class NetworkBootstrap:
    def estimate_risk_from_first_interaction(self, transaction) -> float:
        # Check destination reputation
        dst_risk = self.model.scorer.forward_node(
            self.model.compute_embedding(transaction.dst)
        )
        
        # Check if device is shared with known entities
        shared_device_entities = self.device_index.get_shared(
            transaction.device_fingerprint
        )
        if shared_device_entities:
            # Inherit highest risk from device-sharing cluster
            cluster_risk = max(
                self._get_entity_risk(e) for e in shared_device_entities
            )
            return max(dst_risk, cluster_risk)
        
        return dst_risk
```

### Hybrid Cold-Start System

In practice, all five strategies are combined:

```python
class HybridColdStartSystem:
    def score_new_entity(self, entity, transaction):
        scores = {
            'transfer_learning': self.transfer_bootstrap.bootstrap_score(entity),
            'external_signals': self.external_scorer.score_new_entity(entity.metadata),
            'network_context': self.network_bootstrap.estimate_risk_from_first_interaction(transaction),
            'behavioral': self.behavioral_scorer.score(entity.session_data),
        }
        
        # Weighted ensemble (external signals dominate until history builds)
        weights = {'transfer_learning': 0.2, 'external_signals': 0.4,
                   'network_context': 0.3, 'behavioral': 0.1}
        
        combined = sum(scores[k] * weights[k] for k in scores)
        
        # Apply progressive trust constraints
        tier = self.trust_manager.get_tier(entity.id)
        if combined > tier.get('block_threshold', 0.8):
            return 'BLOCK'
        elif combined > tier.get('review_threshold', 0.5):
            return 'REVIEW'
        return 'ALLOW'
```
...
## 13. Transfer Learning Extensibility
...
## 14. AWS Demo Architecture & Deployment
...
│                                                                          │
│  ┌──────────────┐     ┌──────────────┐     ┌──────────────────────┐     │
│  │ Amazon MSK   │────►│ AWS Lambda   │────►│ Neptune Analytics    │     │
│  │ (Kafka)      │     │ (Ingestion)  │     │ (Graph Storage)      │     │
│  │              │     │              │     │                      │     │
│  └──────────────┘     └──────────────┘     └──────────────────────┘     │
│         ▲                                            │                   │
│         │                                            ▼                   │
│  ┌──────────────┐                          ┌──────────────────────┐     │
│  │ Synthetic    │                          │ SageMaker            │     │
│  │ Data Gen     │                          │ (TGN Training +      │     │
│  │ (Lambda)     │                          │  Real-time Endpoint) │     │
│  └──────────────┘                          └──────────┬───────────┘     │
│                                                       │                  │
│                                                       ▼                  │
│  ┌──────────────┐     ┌──────────────┐     ┌──────────────────────┐     │
│  │ Streamlit /  │◄────│ ElastiCache  │◄────│ Lambda               │     │
│  │ Dashboard    │     │ (Redis)      │     │ (Rules Engine)       │     │
│  │ (ECS)        │     │ Feature Store│     │                      │     │
│  └──────────────┘     └──────────────┘     └──────────────────────┘     │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| Layer | AWS Service | Purpose | Rationale |
|-------|-------------|---------|-----------|
| **Streaming** | Amazon MSK Serverless | Kafka topics for events | Serverless = no cluster management |
| **Ingestion** | Lambda | Transform to UnifiedGraphEvent | Auto-scales, cost-effective |
| **Graph Store** | Neptune Analytics | Graph topology, neighbour index | Purpose-built, openCypher support |
| **Training** | SageMaker Training | Batch TGN training (PyG) | Managed GPU, spot pricing |
| **Inference** | SageMaker Endpoint | Real-time TGN scoring | ml.g5.xlarge (A10G) — p50 <15ms |
| **Feature Store** | ElastiCache (Redis) | Risk scores + embeddings | Sub-ms reads, TTL-based expiry |
| **Rules Engine** | Lambda | Threshold evaluation | Stateless, event-driven |
| **Dashboard** | Streamlit on ECS Fargate | Live visualisation | Rapid development |
| **Data Gen** | Lambda (scheduled) | Synthetic fraud scenarios | Controlled demo narratives |

### SageMaker Endpoint Configuration

```python
# inference.py — SageMaker endpoint container
import torch
from model import HeteroTGNFraudDetector, HeteroNodeRegistry


def model_fn(model_dir):
    """Load model for SageMaker inference."""
    registry = HeteroNodeRegistry.from_schema()
    model = HeteroTGNFraudDetector(registry, edge_feat_dim=20)
...
def predict_fn(input_data, model):
    """Score a transaction batch."""
    src = torch.tensor(input_data['src'], dtype=torch.long)
    dst = torch.tensor(input_data['dst'], dtype=torch.long)
    t = torch.tensor(input_data['timestamps'], dtype=torch.float)
    msg = torch.tensor(input_data['features'], dtype=torch.float)
    etype = torch.tensor(input_data['edge_types'], dtype=torch.long)
    
    with torch.no_grad():
        output = model(src, dst, t, msg, etype)
    
    return {
        'transaction_scores': output['pos_score'].squeeze().tolist(),
        'account_scores': output['node_scores'].squeeze().tolist(),
    }
```

### Demo Scenarios

The synthetic data generator produces controlled fraud patterns:

| Scenario | Pattern | TGN Advantage |
|----------|---------|---------------|
| **Structuring** | 8 × £9,500 transfers in 1 hour | Mean aggregation captures full burst; rules miss sub-threshold individual txns |
| **Account Takeover** | New device → rapid transfers → unfamiliar destinations | Relational attention correlates login + transfer signals across edge types |
| **Mule Network** | Ring of 6 accounts passing money in a circle | Graph neighbourhood topology reveals cycle pattern |
| **False Positive Reduction** | Payroll run (50 legitimate payments) | Learned merchant memory recognises established pattern |

### Cost Estimate (Customer-Facing Demo, 8hr/day)

| Service | Configuration | Monthly Cost |
|---------|---------------|-------------|
| SageMaker Endpoint | ml.g5.xlarge, 8hr/day | ~£280 |
| MSK Serverless | ~1GB throughput/day | ~£15 |
| ElastiCache | r6g.large | ~£90 |
| Lambda | ~100K invocations/day | ~£5 |
| Neptune Analytics | 4 vCPU | ~£180 |
| ECS Fargate (Streamlit) | 0.5 vCPU / 1GB | ~£20 |
| **Total** | | **~£590/month** |

*Cost reduction: Scale SageMaker endpoint to 0 when idle + scheduled MSK = ~60% savings for intermittent demos.*
...
**Objectives**: Single institution deployment, prove value with internal data.

**Activities**:
- Integrate with core banking transaction processing systems
- Build temporal graph from 2+ years historical data
- Train initial HeteroTGN model on internal labelled fraud cases
- Deploy fast-path scoring engine with SageMaker endpoint
- Set up monitoring, alerting, and concept drift detection

**Deliverables**:
- Working fraud detection system (single institution)
- Performance dashboard with real-time metrics
- Analyst investigation tools with graph explainability
- Business case validation with measured lift
...
- p50 <15ms decision latency
- 95% system uptime

### Phase 2: Multi-Bank Network (Months 5–7)

**Objectives**: Connect 2–3 pilot banks, demonstrate cross-institution detection.

**Activities**:
- Implement entity resolution engine (4-tier)
- Deploy federated learning protocols
- Establish cross-bank alert sharing via encrypted channels
- Create governance framework and data sharing agreements
- Pilot dark web monitoring integration

**Deliverables**:
- Multi-bank fraud detection active
- Privacy controls validated by compliance
- Shared intelligence platform operational
...
- 5+ cross-bank fraud rings detected
- 98% system uptime

### Phase 3: Advanced Capabilities (Months 8–10)

**Objectives**: Full system capabilities, scale to additional banks.

**Activities**:
- Deploy behavioural fingerprinting for cold-start handling
- Activate deep-path analysis for complex cases
- Implement all five cold-start strategies
- Add external data sources (dark web, device intelligence)
- Performance tuning and model optimisation

**Deliverables**:
- Complete system operational with all features
- Advanced features active (cold-start, deep path, concept drift)
- Scalable architecture validated at 10x current volume
...
- p50 <12ms fast-path latency
- 99.5% system uptime

### Ongoing Operations

- Model retraining: Triggered by concept drift detection (not scheduled)
- New source integration: 2–3 day warm-start process per new feed
- Performance monitoring: Automated alerting on latency/accuracy degradation
...
|---------------|-----------|
| Technical | Phased rollout with fallback to existing systems; parallel running during transition |
| Business | Pilot program validates business case; quick wins in Phase 1 build confidence |
| Regulatory | Privacy-by-design architecture; regular compliance reviews; audit trail for all decisions |
| Operational | 24/7 monitoring; automated failover; comprehensive testing at each phase |

---

## 16. Business Case & ROI

### Return on Investment

| Metric | Year 1 | Year 2 | Year 3 |
|--------|--------|--------|--------|
| Implementation cost | £1.8–3.0M | £300–500K (ops) | £300–500K (ops) |
| Fraud loss reduction | 35% (£X saving) | 50%+ | 55%+ |
| False positive reduction | 50% | 70%+ | 75%+ |
| Analyst productivity gain | 40% | 60% | 65% |
| Estimated ROI | 2–3x | 5–8x | 8–12x |

*Actual £ figures depend on institution size and current fraud rates. A mid-size bank processing £50B annually with a 3bps fraud rate (£15M losses) would see £5.25M savings in Year 1 against ~£2.5M total cost = 2.1x ROI.*

### Customer Experience Benefits

- **Good customers never notice**: Legitimate transactions approved in <15ms
- **Suspicious activity gets proper scrutiny**: 5% routed to deep path without blocking
- **Fewer false declines**: 50–70% reduction means fewer angry calls
- **Faster resolution**: Graph explainability accelerates investigation from hours to minutes

### Strategic Advantages

- **Network effects**: Each additional institution strengthens detection for all participants
- **Competitive moat**: Temporal graph + heterogeneous TGN is not easily replicated
- **Regulatory compliance**: Built-in explainability satisfies supervisory requirements
- **Future-proof**: Incremental learning means the system improves continuously without costly retraining cycles
...
2. **Kapoor & Narayanan (2023)** — *Leakage and the Reproducibility Crisis in ML-Based Science* (critical reading on temporal data leakage)
3. **NVIDIA Financial Fraud Blueprint** — GNN embeddings + XGBoost ensemble pattern
4. **PyG TGN Documentation** — `torch_geometric.nn.models.tgn.TGNMemory`
5. **TGB Benchmark** — Temporal Graph Benchmark for standardised evaluation
6. **T-GNNExplainer (Xia et al.)** — Model-agnostic explainability for temporal GNNs
7. **Hamilton et al. (2017)** — *Inductive Representation Learning on Large Graphs* (GraphSAGE aggregation strategies)
8. **Xu et al. (2019)** — *How Powerful are Graph Neural Networks?* (GIN; mean vs sum vs max aggregation)
9. **Kim et al. (2024)** — DGraph benchmark for TGN fraud detection

---

*Framework: PyTorch Geometric | Python 3.10+ | AWS (SageMaker, MSK, Neptune, ElastiCache)*
